...
void odomCallback(const std_msgs::String::ConstPtr & msg) {
  ...
  // ROS_INFO_STREAM("got string msg");

  while (std::getline(ss, token, ',')) {
    in[i++] = token;
  }

  // ROS_INFO_STREAM("exploded accross ,");
  ...
  // ROS_INFO_STREAM("converted to double");
  //ROS_INFO_STREAM("Left:" << enL1 << enL2);
...
int main(int argc, char **argv) {
  ...
  ROS_INFO_STREAM("added pubs");

  ros::Rate rate(40);

  while (n.ok()) {
    //ROS_INFO_STREAM("while, spinned once");
